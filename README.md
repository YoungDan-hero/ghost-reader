...
- **幽灵悬浮** —— `v` 进入幽灵模式，窗口边框、标题栏、底色全部消失，只剩几行小说文字悬浮在桌面上，可拖到任意文档上"融为一体"，极致隐蔽。
- **ghost 文字精调** —— 幽灵模式下用快捷键实时调整字号、行高、字间距，`c` 一键拾取屏幕上任意像素颜色套到文字上，与目标文档格式完全对齐；所有样式自动记住，下次打开仍在。
...
- **中文输入兼容** —— 跳章 / 搜索面板完整支持拼音、双拼等中文输入法，输入不卡、候选框不丢。
...
> 幽灵模式下的样式与颜色会**自动缓存**到本地，重启应用后保持上次设置。
...
- 透明窗口：`transparent: true` + `backgroundColor: '#00000000'` 实现幽灵悬浮
- 屏幕取色：`desktopCapturer` 截屏 + `nativeImage.toBitmap()` 读取像素
- 全局逃生键：`globalShortcut`
- 主题系统：CSS 变量 + `data-theme` 切换，`localStorage` 持久化
- 中文输入兼容：`isComposing` 守卫 + DOM 不重建（输入框与列表分离）
- 持久化：用户数据目录 JSON + `localStorage`（主题 / ghost 样式）
- 零运行时第三方依赖，纯原生实现
